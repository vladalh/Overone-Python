...
if dict_save_path != "":
    file = open(f"{dict_save_path}movies.{file_format}", "w", encoding="utf-8")

else:
    file = open(f"movies.{file_format}", "w", encoding="utf-8")
...
if dict_save_path != 0:
    file = open(f"{dict_save_path}movies.{file_format}", "r", encoding="utf-8")

else:
    file = open(f"movies.{file_format}", "r", encoding="utf-8")
        ...
        if person.isdigit():
            if int(person) > 1895:
                ...
                if int(person) <= int(cin2):
                    ...
                    msg = f"list of films for a given request :{criterion_list}"
...
if file_format == 'json':
    ...
    for cin in json_data:
        if person.isdigit():
            ...
            for l in json_data[cin]["genre"]:
                if person == l:
                    criterion_list.append(json_data[cin])

                if person in json_data[cin]["country"]:
                    criterion_list.append(json_data[cin])
...
if result_path != "":
    file = open(f"{result_path}/result_mmovies.{file_format}", "w", encoding="utf-8")

else:
    file = open(f"result_mmovies.{file_format}", "w", encoding="utf-8")
